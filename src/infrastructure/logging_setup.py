...
def setup_logging(log_level: str = "INFO", service_name: Optional[str] = None) -> None:
    ...
    formatter = JsonFormatter(fmt=fmt, rename_fields={"levelname": "level", "asctime": "timestamp"}, timestamp=True)
    ...
    for noisy in ["motor", "pymongo", "httpx", "httpcore", "sentence_transformers",
                  "transformers", "qdrant_client", "urllib3", "asyncio"]:
